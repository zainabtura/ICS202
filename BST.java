...
public class BST<T extends Comparable<? super T>> {
    ...
    // 1. Count the number of nodes in the binary tree
    public int count() {
        return count(root);
    }
    ...
    // 2. Determine if a node with a given value is a leaf
    public boolean isLeaf(T value) {
        return isLeaf(root, value);
    }

    private boolean isLeaf(BSTNode<T> node, T value) {
        if (node == null) {
            return false;
        }
        if (node.el.equals(value)) {
            return (node.left == null && node.right == null);
        }
        return isLeaf(node.left, value) || isLeaf(node.right, value);
    }

    // 3. Count the number of leaves in the binary tree

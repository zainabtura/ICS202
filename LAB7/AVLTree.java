public class AVLTree<T extends Comparable<? super T>> extends BST<T> {
   ...
   private AVLTree<T> getRightAVL() {
    ...
    public void deleteAVL(T el) {
        deleteAVLHelper(root, el);
        balance();
    }
    private BSTNode<T> deleteAVLHelper(BSTNode<T> node, T el) {
        if (node == null) {
            return null;
        }

        int compareResult = el.compareTo(node.el);
        if (compareResult < 0) {
            node.left = deleteAVLHelper(node.left, el);
        } else if (compareResult > 0) {
            node.right = deleteAVLHelper(node.right, el);
        } else {
            if (node.left != null && node.right != null) {
                node.el = findMax(node.left).el;
                node.left = deleteAVLHelper(node.left, node.el);
            } else {
